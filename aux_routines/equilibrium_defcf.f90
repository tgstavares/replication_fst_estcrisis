...
contains
  ...
  subroutine get_equil(par,m,showoutput,getinterpcoeff)
    ...
    do while(difv.gt.erro4)
       ...
       ! if(m%iter.gt.3.and.naccel.gt.0.and.m%jflag.eq.0.and.m%iter.lt.6)then
       !    do i=1,naccel
       ...
       !                do ip1=1,np
       !                   do ik1=1,nk
       !                      call gget_ev(ik1,ip1,iz0,ia0,ix0,m%pz,m%pa,m%px,m%vg,m%evg(ik1,ip1,iz0,ia0,ix0),m%qbg(ik1,ip1,iz0,ia0,ix0))
       !                   end do
       !                end do
       !             end do
       !          end do
       ...
       !                      call cashflowr(par,m%tau,m%taudiv,m%k(ik0),m%p(ip0),m%z(iz0),m%a(ia0),m%k(ik1),m%p(ip1),m%qbg(ik1,ip1,iz0,ia0,ix0),cashflow)
             ...
             do iz0=1,nz
                ...
                do ip1=1,np
                   do ik1=1,nk
                      ...
                      call gget_ev(par%delta,m%beta,m%dwl,m%k(ik1),m%p(ip1),m%z,m%a,ik1,ip1,iz0,ia0,ix0,m%pz,m%pa,m%px,m%vg,m%evg(ik1,ip1,iz0,ia0,ix0),m%qbg(ik1,ip1,iz0,ia0,ix0))
                      !call gget_ev(ik1,ip1,iz0,ia0,ix0,m%pz,m%pa,m%px,m%vg,m%evg(ik1,ip1,iz0,ia0,ix0),m%qbg(ik1,ip1,iz0,ia0,ix0))

                      !print*,m%p(ip1),m%evg(ik1,ip1,iz0,ia0,ix0),m%qbg(ik1,ip1,iz0,ia0,ix0)/(1d0+r)
                   end do                   
                end do
             end do
          end do
       ...
       !          do ik1=nk/2,nk
       !             do ip1=1,np
       !                call gget_ev(par%delta,m%k(ik1),m%p(ip1),m%z,m%a,ik1,ip1,iz0,ia0,ix0,m%pz,m%pa,m%px,m%vg,m%evg(ik1,ip1,iz0,ia0,ix0),m%qbg(ik1,ip1,iz0,ia0,ix0))
       !                print*,m%k(ik1),m%p(ip1),m%evg(ik1,ip1,iz0,ia0,ix0),m%qbg(ik1,ip1,iz0,ia0,ix0)/(1d0+r)
       !             end do
       !             stop
       !          end do
       !       end do
       ...
       !$omp parallel do collapse(5) private(ix0,ia0,iz0,ip0,ik0,qb0)
       do ix0=1,nx
          do ia0=1,na
             do iz0=1,nz
                do ip0=1,np
                   do ik0=1,nk
                      call get_vc(par,m%sparam,m%r,m%beta,m%tau,m%taudiv,m%k(ik0),m%p(ip0),m%z(iz0),m%a(ia0),m%k,m%p,m%evg(:,:,iz0,ia0,ix0),m%qbg(:,:,iz0,ia0,ix0), &
                      ...
                      ! cumula = max(cumula,abs(&
                      !      m%vg( ik0+nk*(ip0-1+np*(iz0-1+nz*(ia0-1+na*(ix0-1))))) - &
                      !      m%vc( ik0+nk*(ip0-1+np*(iz0-1+nz*(ia0-1+na*(ix0-1))))) &
                      !      ))
                      ! write(*,'(3i10,10f15.4)')iz0,ip0,ik0, &
                      !      cumula,m%vg( ik0+nk*(ip0-1+np*(iz0-1+nz*(ia0-1+na*(ix0-1))))), &
                      !      m%vc( ik0+nk*(ip0-1+np*(iz0-1+nz*(ia0-1+na*(ix0-1))))), &
                      !      m%k(m%ikp(ik0+nk*(ip0-1+np*(iz0-1+nz*(ia0-1+na*(ix0-1)))))), &
                      !      m%p(m%ipp(ik0+nk*(ip0-1+np*(iz0-1+nz*(ia0-1+na*(ix0-1))))))
       ...
       difv = sum(abs(m%vc-m%vg)/abs(m%vg+1d0))/dble(nk*np*nz*na*nx)
       ...
       do ix0=1,nx
          do ia0=1,na
             do iz0=1,nz
                ...
                m%coefqbg(1,1,:,:,iz0,ia0,ix0) = m%qbg(    :,:,iz0,ia0,ix0)
                call bcspline(m%k,nk,m%p,np, m%coefqbg(:,:,:,:,iz0,ia0,ix0),nk,0,bcxmin,0,bcxmax,0,bcymin,0,bcymax,wk,5*max(nk,np),ilinx,iliny,ier)
             end do
          end do
       end do
    end if
