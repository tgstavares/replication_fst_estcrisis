...
contains
  ...
  subroutine get_equil(par,m,showoutput,getinterpcoeff)
    ...
    integer i,ik0,ip0,iz0,ia0,ix0,ik1,ip1,ix1,iz1,ia1
    real(dp) difv,cf1,cf2,k0,p0,z0,a0,ve
    ...
    do ia0=1,na
       do iz0=1,nz
          do ip0=1,np
             do ik0=1,nk
                !call cf1f(par,m%tau,m%k(ik0),m%p(ip0),m%z(iz0),m%a(ia0),cf1)
                k0 = m%k(ik0)
                p0 = m%p(ip0)
                z0 = m%z(iz0)
                a0 = m%a(ia0)
                m%vx(ik0+nk*(ip0-1+np*(iz0-1+nz*(ia0-1)))) = &
    ...
    do while(difv.gt.erro4)
       ...
       if(m%iter.gt.3.and.naccel.gt.0.and.m%jflag.eq.0.and.m%iter.lt.11)then
          do i=1,naccel
             ...
             do ix0=1,nx
                do ia0=1,na
                   do iz0=1,nz
                      do ip1=1,np
                         do ik1=1,nk
                            call gget_ev(ik1,ip1,iz0,ia0,ix0,m%pz,m%pa,m%px,m%vg,m%vx,m%evg(ik1,ip1,iz0,ia0,ix0))
                         end do
                      end do
                   end do
                end do
             ...
             !$acc parallel loop collapse(5) private(ix0,ia0,iz0,ip0,ik0,ik1,ip1,cf1,cf2)
             !$omp parallel do collapse(5) private(ix0,ia0,iz0,ip0,ik0,ik1,ip1,cf1,cf2)
             do ix0=1,nx
                do ia0=1,na
                   do iz0=1,nz
                      do ip0=1,np
                         do ik0=1,nk
                            ...
                            call cf1f(par,m%tau,m%k(ik0),m%p(ip0),m%z(iz0),m%a(ia0),cf1)
                            call cf2f(par,m%tau,m%k(ik0),m%k(ik1),m%p(ip1),cf2)
                            m%vg(ik0+nk*(ip0-1+np*(iz0-1+nz*(ia0-1+na*(ix0-1))))) = cf1+cf2 + beta*m%evg(ik1,ip1,iz0,ia0,ix0)
                         end do
                      end do
                   end do
                end do
       ...
       do ix0=1,nx
          do ia0=1,na
             do iz0=1,nz
                do ip0=1,np
                   do ik0=1,nk
                      call get_vc(par,m%sparam,m%tau,m%taudiv,m%k(ik0),m%p(ip0),m%z(iz0),m%a(ia0),m%k,m%p,m%evg(:,:,iz0,ia0,ix0), &
    ...
    !                z0 = m%z(iz0)
    !                a0 = m%a(ia0)
    !                call get_ve(par,m%sparam,m%tau,m%taudiv,k0,p0,z0,a0,m%k,m%p,m%evg(:,:,iz0,ia0,ix0),ve,ik1,ip1)
    !                write(*,'(10f15.4)')z0,z0,p0,k0,&
    !                     m%vg(ik0+nk*(ip0-1+np*(iz0-1+nz*(ia0-1+na*(ix0-1))))), &
    !                     m%evg(ik0,ip0,iz0,ia0,ix0),ve,m%k(ik1),m%p(ip1)
