...
contains
    ...
    do t=1,tsimul
       ...
       do n=1,nsimul
          ...
          if(t0.le.0.5d0)then
             ...
             call cashflowr(par,m%r,m%tau,m%taudiv,k0,p0,z0,a0,k1,p1,div0)
          end if
          ...
          data(n,t,25) = m%tau * exp(z0+a0) * k0**theta - m%tau*par%delta*k0 - m%tau*m%r*p1*k1/(1d0+m%r)
          !data(n,t,25) = p1*k1/k0
       !    !if(par%sparam2*exp(rhoz*z0+sigmaz**2*0.5d0 + rhoa*a0+sigmaa**2*0.5d0) * k1**theta * k1**(-1).gt.par%sparam1)then
       !    if(par%sparam2*exp(rhoz*z0+sigmaz**2*0.5d0 + rhoa*a0+sigmaa**2*0.5d0) * k0**theta * k0**(-1).gt.par%sparam1)then
       !       data(n,t,25) = 1.0d0
       !    else
       !       data(n,t,25) = 0.0d0
       !    end if
  ...
  subroutine get_stats(data,nstats,stats,showoutput,cond)
    ...
    !call mean(real(pack(data(:,:,25),mask)),n,0,e_bp)
    ...
    !call sampp(real(pack(data(:,:,25),mask)),n,0.5,0,m_bp)
    !call sampp(real(pack(data(:,:,25),mask)),n,0.66,0,q66_bp)    

    call sd(real(pack(data(:,:,12),mask)),n,0,sd_ik)

    call sd(real(pack(data(:,:, 7),mask)),n,0,sd_bp)
    !call sd(real(pack(data(:,:,25),mask)),n,0,sd_bp)
    ...
    a_sales = real(sum(pack(data(:,:,25),mask)))/real(sum(pack(data(:,:,18),mask)))
    ...
    !call cov(real(pack(data(:,:,25),mask)),real(pack(data(:,:,14),mask)) ,n,0,covari)
    !call var(real(pack(data(:,:,25),mask)),n,0,vari)

    slope = covari/vari

    !write(*,'(a15,f15.7)')'extra stat',real(sum(pack(data(:,:,25),mask)))/real(n)
