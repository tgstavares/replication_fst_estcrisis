...
contains
  ...
  subroutine get_simulations(par,m,nsimul,tsimul,nvars,shocksz,shocksa,shocksx,data,initconds,iapath,ixpath,replace_exit)
    ...
    do t=1,tsimul
       ...
       !$omp parallel do collapse(1) private(n,k0,p0,z_1,t0,k_1,inv_1,iaux,fcumulz,iz0,i,z0,v0,k1,p1,qq,inv0,div0,cashflow,nworth)
       do n=1,nsimul
          ...
          if(t0.le.0.5d0)then
             ...
             qq = qq/(1d0+m%r)
             nworth= exp(z0+a0) * k0**theta - p0*k0 + m%dwl*(1d0-par%delta)*k0 - kappa
          end if
          if(v0.lt.0d0.or.div0.lt.0d0.or.nworth.lt.0d0) t0 = -1d0 ! MARK CASE IF EXIT
       ...
       if(iflag_replace.eq.1)then
          ...
          do n=1,nsimul
             if(data(n,t,9).le.0.0d0) then
                ...
                if(v0.lt.0d0.or.div0.lt.0d0.or.nworth.lt.0) stop 'FATAL ERROR: Industry collapse by default'
                data(n,t,1)  = dble(n)
                data(n,t,2)  = dble(t)
                data(n,t,3)  = k0
                data(n,t,4)  = p0
