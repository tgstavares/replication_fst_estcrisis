...
contains
  ...
  subroutine get_simulations(par,m,nsimul,tsimul,nvars,shocksz,shocksa,shocksx,data,initconds,iapath,ixpath,replace_exit)
    ...
    real(dp) ek,ep,ez,utilization,depreciation
    ...
    do t=1,tsimul
       ...
       !$omp parallel do collapse(1) private(n,k0,p0,z_1,t0,k_1,inv_1,iaux,fcumulz,iz0,i,z0,v0,k1,p1,qq,inv0,div0,cashflow,utilization,depreciation)
       do n=1,nsimul
          ...
          ! SIMULATION OUTCOMES
          if(t0.le.0.5d0)then
             utilization = ustar(par%delta,z0+a0,k0)
             depreciation= dep(par%delta,utilization)
             k1   = k0
             p1   = p0
             qq   = 0d0
             div0 = -1d0
          ...
          data(n,t,25) = utilization
          data(n,t,26) = depreciation
          !if(n.eq.33) write(*,'(2i5,10f15.4)')n,t,z0,a0,k0,p0,inv0,inv0/k0,div0,v0,ustar(par%delta,z0+a0,k0)
       end do
       !$omp end parallel do
  ...
  subroutine get_stats(data,nstats,stats,showoutput,cond)
    ...
    real mean_a,mean_z,var_z,agg_profits,agg_capital,e_ut,e_dp
    ...
    slope = covari/vari
    call mean(real(pack(data(:,:,25),mask)),n,0,e_ut)
    call mean(real(pack(data(:,:,26),mask)),n,0,e_dp)
    ...
    stats(31)= e_b;	  stats(32)= slope;	stats(33)= e_ut
    stats(34)= e_dp;
