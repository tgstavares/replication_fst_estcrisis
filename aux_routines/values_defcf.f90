...
contains
  ...
  subroutine cashflowr(par,r,tau,taudiv,k0,p0,z0,a0,k1,p1,qb,cf)
    ...
    real(dp), intent(in) ::r,tau,taudiv,k0,p0,z0,a0,k1,p1,qb
  ...
  subroutine gget_ev(delta,beta,dwl,k1,p1,z,a,ik1,ip1,iz0,ia0,ix0,prbz,prba,prbx,vg,ev,qb)
    implicit none
    real(dp), intent(in) ::delta,beta,dwl,k1,p1,z(nz),a(na)
    ...
    integer iz1,ia1,ix1
    real(dp) vc,prob,a1,z1,aux1,aux2
    !$acc routine
    qb = 0d0
    ev = 0d0
    ...
    do ix1=1,nx
       do ia1=1,na
          do iz1=1,nz
             ...
             ev    = ev + prob * vc
             if(exp(z1+a1)*k1**theta - p1*k1 + dwl*(1d0-delta)*k1 - kappa.ge.0d0)then
                aux1 = aux1 + prob
                qb = qb + prob
             else
                aux2 = aux2 + prob
                qb = qb + prob*min(1d0,rec*(exp(z1+a1)*k1**theta+dwl*(1d0-delta)*k1)/(k1*p1))
             end if
          end do
       end do
    end do
    ...
    !write(*,'(8f12.4)')p1,qb,qb*k1*p1,aux1,aux2
  end subroutine gget_ev

  
  ! ROUTINES FOR VECTORIZED ARRAYS
  subroutine gget_probdef(delta,beta,dwl,k1,p1,z,a,ik1,ip1,iz0,ia0,ix0,prbz,prba,prbx,vg,qb)
    implicit none
    real(dp), intent(in) ::delta,beta,dwl,k1,p1,z(nz),a(na)
    integer,  intent(in) ::ik1,ip1,iz0,ia0,ix0
    real(dp), intent(in) ::prbz(nz*nz),prba(na*na),prbx(nx*nx),vg(nk*np*nz*na*nx)
