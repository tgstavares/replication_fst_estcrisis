...
contains
  ...
  subroutine cashflowr(par,tau,taudiv,k0,p0,z0,a0,k1,p1,cf)
    ...
    real(dp) adj,adjs,inv0,inv0_til,utilization,depreciation
    !$acc routine
    utilization = ((1d0-tau)*exp(z0+a0)*k0**(theta-1)/eta_norm)**(1d0/(eta-1d0))
    depreciation= par%delta + (eta_norm/eta)*(utilization**eta - 1d0)

    !inv0 = k1-k0*(1d0-par%delta)
    inv0 = k1-k0*(1d0-depreciation)
    inv0_til = k1-k0*(1d0-par%delta)
    
    adj  = 0.5d0 * par%a * (inv0_til * k0**(-1))**2 * k0  + par%gamma*k0
    adjs = 0d0
    if(abs(inv0_til).lt.1d-7)	adj  = 0d0
    if(inv0_til.lt.-1d-7) 	adjs =-par%ps*inv0
    
    ! CAHSFLOW WITH TAXES
    ! cf   = (1d0-taudiv)*((1d0-tau)*exp(z0+a0) * k0**theta + tau*par%delta*k0 - inv0 - adj - adjs + &
    !      (1d0+r*(1d0-tau))**(-1) * p1*k1 - p0*k0)

    cf   = (1d0-taudiv)*((1d0-tau)*exp(z0+a0) * utilization * k0**theta + tau*depreciation*k0 - inv0 - adj - adjs + &
         (1d0+r*(1d0-tau))**(-1) * p1*k1 - p0*k0)
  ...
  real(dp) function dep(delta,u)
    implicit none
    real(dp), intent(in)::delta,u
    dep = delta + (eta_norm/eta)*(u**eta - 1d0)
  end function dep

  real(dp) function ustar(delta,z,k)
    implicit none
    real(dp), intent(in)::delta,z,k
    ustar = (exp(z)*k**(theta-1)/eta_norm)**(1d0/(eta-1d0))
  end function ustar
