program gplot
  ...
  open(1,file='data/e_p.txt',position="rewind");read(1,*)e_p;close(1)
  ...
  open(4,file='plot_data/Policyfnz_baseline.txt',position="rewind")
  ...
  do i=1,2
     ...
     if(i.eq.1)then
        !iaux = minloc(abs(m%p  - e_p*0.93)); ip0  = iaux(1)
        iaux = minloc(abs(m%p  - 0.90d0)); ip0  = iaux(1)	! 2sd of p
     elseif(i.eq.2)then
        !iaux = minloc(abs(m%p  - e_p*1.07)); ip0  = iaux(1)
        iaux = minloc(abs(m%p  - 1.10d0)); ip0  = iaux(1)	! 2sd of p
     end if
     p0 = m%p(ip0)
     print*,i,p0
     ...
     do ik0=1,nk
        ...
        write(1,'(15f20.6)')k0,p0,z0,k1,p1*k1,inv0,inv0/k0,p1,div0,val0,a0,plimit
     end do
     write(1,'(//)')

     ! FIX SOME K - CHANGE Z

     iaux = minloc(abs(m%k  - 200d0));  ik0  = iaux(1) ! 1sd of k
     k0 = m%k(ik0)
     do iz0=1,nz
        z0 = m%z(iz0)
        if(iflag_searchwide.eq.1)then
     ...
     iaux = minloc(abs(m%k  - e_k*0.7d0));  ik0  = iaux(1)
     k0 = m%k(ik0)
     k1 = k0
     z0 = m%z(iz0)
     do ip0=1,np        
