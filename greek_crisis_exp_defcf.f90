program greek_crisis_exp
  ...
  use openacc
#endif
  use prec; use parameters; use globals; use process; use equilibrium; use simulation; use values
  implicit none

  type(model) m,m_2,m_1,m0,m1,m2,m3,m4       ,m5
  type(param) par
  integer, parameter:: tsimul = 114, nsimul = 50000, nvars = 25
  integer n,t,tdiscard,tsimul1,iloc(1),tt0,tt1,iflag_turnashocks,iflag_turnxshocks,iflag_creditshock,iflag_saved,ik1,ip1,ix0,iz0,ia0
  ...
  real(Dp) spread,conta, div00
  real scale_param
  real st1,st2,st3,st4,st5
  integer num2
  ...
  if(iflag_creditshock.eq.0)then
     ...
     ! ! r shock
     ! m0%r  = 0.030d0; m0%beta = 1d0/(1d0+m0%r)
     ! m1%r  = 0.030d0; m1%beta = 1d0/(1d0+m1%r)
     ! m2%r  = 0.030d0; m2%beta = 1d0/(1d0+m2%r)
     ! m3%r  = 0.030d0; m3%beta = 1d0/(1d0+m3%r)
     ! m4%r  = 0.030d0; m4%beta = 1d0/(1d0+m4%r)

     ! ! r shock beta constant - acc lev
     ! m_2%r = 0.0250d0
     ! m_1%r = 0.0260d0
     ! m0%r  = 0.0300d0
     ! m1%r  = 0.0300d0
     ...
     ! r shock beta constant - prov lev
     m_2%r = 0.0250d0
     m_1%r = 0.0265d0
     m0%r  = 0.0325d0
     m1%r  = 0.0312d0
     ...
     do tt0 = 108,114
        ...
        case (108)
           ...
           write(*,'(a40,4f12.4,i10)')'##### Getting simulation for sparam: ',m_2%sparam,m_2%tau,m_2%r,m_2%beta,tt0
        ...
        case (109)
           ...
           write(*,'(a40,4f12.4,i10)')'##### Getting simulation for sparam: ',m_1%sparam,m_1%tau,m_1%r,m_1%beta,tt0
        ...
        case (110)
           ...
           write(*,'(a40,4f12.4,i10)')'##### Getting simulation for sparam: ',m0%sparam,m0%tau,m0%r,m0%beta,tt0
        ...
        case(111)
           ...
           write(*,'(a40,4f12.4,i10)')'##### Getting simulation for sparam: ',m1%sparam,m1%tau,m1%r,m1%beta,tt0
        ...
        case(112)
           ...
           write(*,'(a40,4f12.4,i10)')'##### Getting simulation for sparam: ',m2%sparam,m2%tau,m2%r,m2%beta,tt0
        ...
        case(113)
           ...
           write(*,'(a40,4f12.4,i10)')'##### Getting simulation for sparam: ',m3%sparam,m3%tau,m3%r,m3%beta,tt0
        ...
        case(114)
           ...
           write(*,'(a40,4f12.4,i10)')'##### Getting simulation for sparam: ',m4%sparam,m4%tau,m4%r,m4%beta,tt0
        ...
        case (114)
           ...
           m5=m4
           !m5%r = 0.03375d0
           call get_equil(par,m5,showoutput='y')
           call get_simulations(par,m5,nsimul,1,nvars,shocksz(:,tt0),shocksa(tt0),shocksx(tt0),data(:,tt0,:),initconds=initconds,iapath=iapath(tt0),ixpath=ixpath(tt0),replace_exit='n')
           allocate(mask2(nsimul))
           mask2 = data(:,114,9).gt.0.5d0
           num2  = count(mask2)
           call mean( real(pack(data(:,114,19),mask2)),num2    ,0,st1)
           call sampp(real(pack(data(:,114,19),mask2)),num2,0.5,0,st2)
           st3 = real(count(pack(data(:,114,19),mask2).lt.0.00))/real(num2)
           call mean( real(pack(data(:,114,25),mask2)),num2    ,0,st4)
           call mean( real(pack(data(:,114,7),mask2)),num2    ,0,st5)
           deallocate(mask2)
           write(*,'(a10,i10,5f12.5)')'==>',num2,st1,st2,st3,st4,st5
           do n=1,nsimul
              data(n,1,6) = data(n,114,6)
              data(n,1,7) = data(n,114,7)
           end do
           ...
           do n=1,nsimul
              iloc = minloc(abs(data(n,1,6) -m4%k)); ik1 = iloc(1)
              iloc = minloc(abs(data(n,1,7) -m4%p)); ip1 = iloc(1)
              iloc = minloc(abs(data(n,114,5) -m4%z)); iz0 = iloc(1)
              iloc = minloc(abs(data(n,114,21)-m4%a)); ia0 = iloc(1)
              call cashflowr(par,m4%r,m4%tau,m4%taudiv, &
                   data(n,114,3), & !k0
                   data(n,114,4), & !p0
                   data(n,114,5), & !z0
                   data(n,114,21),& !a0                   
                   data(n,1,6), & !k1
                   data(n,1,7), & !p1                   
                   m4%qbg(ik1,ip1,iz0,ia0,1),&                   
                   div00)
              data(n,1,19) = div00
              data(n,1,25) = m4%qbg(ik1,ip1,iz0,ia0,1)/(1d0+m4%r)
              data(n,1,24) = data(n,1,7)
           end do
           allocate(mask2(nsimul))
           mask2 = data(:,114,9).gt.0.5d0
           num2  = count(mask2)
           call mean( real(pack(data(:,1,19),mask2)),num2    ,0,st1)
           call sampp(real(pack(data(:,1,19),mask2)),num2,0.5,0,st2)
           st3 = real(count(pack(data(:,1,19),mask2).lt.0.00))/real(num2)
           call mean( real(pack(data(:,1,25),mask2)),num2    ,0,st4)
           call mean( real(pack(data(:,1,7),mask2)),num2    ,0,st5)
           deallocate(mask2)           
           write(*,'(a10,i10,5f12.5)')'==>',num2,st1,st2,st3,st4,st5
           ...
           ! conta =0d0
           ! do n=1,nsimul
           !    if(data(n,114,9).gt.0.5d0)then
           !       iloc = minloc(abs(data(n,114,6) -m4%k)); ik1 = iloc(1)
           !       iloc = minloc(abs(data(n,114,7) -m4%p)); ip1 = iloc(1)
           !       iloc = minloc(abs(data(n,114,5) -m4%z)); iz0 = iloc(1)
           !       iloc = minloc(abs(data(n,114,21)-m4%a)); ia0 = iloc(1)

           !       spread = spread + m5%qbg(ik1,ip1,iz0,ia0,1)
           !       !spread = spread + (1d0+m4%r)/m5%qbg(ik1,ip1,iz0,ia0,1)-1d0 - m4%r
           !       !spread = spread + 1d0/data(n,114,25)-1d0 - m5%r
           !       conta  = conta + 1d0
  ...
  !open(3,file='plot_data/Distributions_cal_estimation_PUT_defcf.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_PUT_dr_defcf.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_PUTebkacc_drb_defcf.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_PUTebkprov_drb_defcf.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_PUTebkacc_dwl_defcf.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_PUTebkprov_dwl_defcf.txt',position="rewind")
  ...
  do t=101,tsimul ! SEE LEGACY CODES FOR CONSISTENCY
     do n=1,nsimul
        write(3,'(2i10,21f16.5)')t,n,m%sparam, &
             ...
             data(n,t,6),data(n,t,7),data(n,t,9),data(n,t,25)
     end do
  end do
  write(3,'(//)')
  close(3)
