program greek_crisis_exp
  ...
  type(param) par
  integer, parameter:: tsimul = 114, nsimul = 90000, nfirms=40000, nentrants=1600, nvars = 27
  ...
  if(iflag_turnashocks.eq.1)then
     ...
     iloc = minloc(abs(log(1.010d0) - m%a)); iapath(108) = iloc(1)
     iloc = minloc(abs(log(0.956d0) - m%a)); iapath(109) = iloc(1)                
     iloc = minloc(abs(log(0.834d0) - m%a)); iapath(110) = iloc(1)
     iloc = minloc(abs(log(0.794d0) - m%a)); iapath(111) = iloc(1)
     iloc = minloc(abs(log(0.777d0) - m%a)); iapath(112) = iloc(1)
     iloc = minloc(abs(log(0.885d0) - m%a)); iapath(113) = iloc(1)
     iloc = minloc(abs(log(0.934d0) - m%a)); iapath(114) = iloc(1)        
  end if
  ...
  if(iflag_creditshock.eq.0)then
     tt1 = 5+2+5
     ! BEFORE 2003
     call get_simulations(par,m,nfirms,nsimul,tsimul-tt1,nvars,shocksz(:,1:tsimul-tt1),shocksa(1:tsimul-tt1),shocksx(1:tsimul-tt1),data(:,1:tsimul-tt1,:),iapath=iapath(1:tsimul-tt1),ixpath=ixpath(1:tsimul-tt1),replace_exit='y',endog_entry='y')
     ! FROM 2003-2009
     do tt0=103,109
        ...
        call get_simulations(par,m,nfirms,nsimul,1,nvars,shocksz(:,tt0),shocksa(tt0),shocksx(tt0),data(:,tt0,:),initconds=initconds,iapath=iapath(tt0),ixpath=ixpath(tt0),replace_exit='y',endog_entry='y',massentrants=nentrants)
     end do
     ! FROM 2010-2014
     do tt0 = 110,114
     ...
     !    mask = data(:,tt0:tt0,9).gt.1.5d0
     !    call get_stats(data(:,tt0:tt0,:),nstats,statistics,showoutput='n',cond=mask)     
     !    write(*,'(1i4,1i8,22f11.4)')tt0,count(mask),m%a(iapath(tt0)),m%xsigmaz(ixpath(tt0)), &
     ...
     !         statistics(27)/scale_param,statistics(33),statistics(34)
     !    deallocate(mask)
     ! end do
     ...
     ee = -1.0d0	! TO SIMULATE MORE PRONOUNCED CREDIT SUPPLY SHOCKS

     ! ! SPARAM UNDER BASELINE
     ! m0%sparam = sparam*(1d0 - 0.012d0*(1d0+ee))
     ! m1%sparam = sparam*(1d0 - 0.028d0*(1d0+ee))
     ! m2%sparam = sparam*(1d0 - 0.060d0*(1d0+ee))
     ! m3%sparam = sparam*(1d0 - 0.080d0*(1d0+ee))
     ! m4%sparam = sparam*(1d0 - 0.085d0*(1d0+ee))     

     ! ! SPARAM UNDER PROVISION
     ! m0%sparam = sparam*(1d0 - 0.035d0*(1d0+ee))
     ! m1%sparam = sparam*(1d0 - 0.075d0*(1d0+ee))    
     ! m2%sparam = sparam*(1d0 - 0.140d0*(1d0+ee))
     ! m3%sparam = sparam*(1d0 - 0.190d0*(1d0+ee))
     ! m4%sparam = sparam*(1d0 - 0.215d0*(1d0+ee))

     ! ! SPARAM UNDER BASELINE AGG
     ! m0%sparam = sparam*(1d0 - 0.027d0*(1d0+ee))
     ! m1%sparam = sparam*(1d0 + 0.010d0*(1d0+ee))
     ! m2%sparam = sparam*(1d0 - 0.077d0*(1d0+ee))
     ! m3%sparam = sparam*(1d0 - 0.040d0*(1d0+ee))
     ! m4%sparam = sparam*(1d0 - 0.119d0*(1d0+ee))     

     ! SPARAM UNDER PROVISION AGG
     m0%sparam = sparam*(1d0 - 0.040d0*(1d0+ee))
     m1%sparam = sparam*(1d0 - 0.041d0*(1d0+ee))    
     m2%sparam = sparam*(1d0 - 0.150d0*(1d0+ee))
     m3%sparam = sparam*(1d0 - 0.150d0*(1d0+ee))
     m4%sparam = sparam*(1d0 - 0.235d0*(1d0+ee))
     ...
     do tt0 = 110,114
        ...
        select case (tt0)
        case (108)
           call get_simulations(par,m_2,nfirms,nsimul,1,nvars,shocksz(:,tt0),shocksa(tt0),shocksx(tt0),data(:,tt0,:),initconds=initconds,iapath=iapath(tt0),ixpath=ixpath(tt0),replace_exit='y',endog_entry='y',massentrants=nentrants)
        case (109)
           call get_simulations(par,m_1,nfirms,nsimul,1,nvars,shocksz(:,tt0),shocksa(tt0),shocksx(tt0),data(:,tt0,:),initconds=initconds,iapath=iapath(tt0),ixpath=ixpath(tt0),replace_exit='y',endog_entry='y',massentrants=nentrants)           
        case (110)
           call get_simulations(par,m0,nfirms,nsimul,1,nvars,shocksz(:,tt0),shocksa(tt0),shocksx(tt0),data(:,tt0,:),initconds=initconds,iapath=iapath(tt0),ixpath=ixpath(tt0),replace_exit='y',endog_entry='y',massentrants=nentrants)
        case (111)
           call get_simulations(par,m1,nfirms,nsimul,1,nvars,shocksz(:,tt0),shocksa(tt0),shocksx(tt0),data(:,tt0,:),initconds=initconds,iapath=iapath(tt0),ixpath=ixpath(tt0),replace_exit='y',endog_entry='y',massentrants=nentrants)
        case (112)
           call get_simulations(par,m2,nfirms,nsimul,1,nvars,shocksz(:,tt0),shocksa(tt0),shocksx(tt0),data(:,tt0,:),initconds=initconds,iapath=iapath(tt0),ixpath=ixpath(tt0),replace_exit='y',endog_entry='y',massentrants=nentrants)
        case (113)
           call get_simulations(par,m3,nfirms,nsimul,1,nvars,shocksz(:,tt0),shocksa(tt0),shocksx(tt0),data(:,tt0,:),initconds=initconds,iapath=iapath(tt0),ixpath=ixpath(tt0),replace_exit='y',endog_entry='y',massentrants=nentrants)           
        case (114)
           call get_simulations(par,m4,nfirms,nsimul,1,nvars,shocksz(:,tt0),shocksa(tt0),shocksx(tt0),data(:,tt0,:),initconds=initconds,iapath=iapath(tt0),ixpath=ixpath(tt0),replace_exit='y',endog_entry='y',massentrants=nentrants)           
        end select
  ...
  write(*,'(1a4,1a8,22a11)')'t','obs','a','x','e_ik','m_ik','a_ik', &
       ...
       'ina','spike','e_k', &
       'a_salesk','sd_mpk','a_sales','a_inv','a_b','rexit','rentry'
  ...
  do t=101,tsimul
     ...
     allocate(mask(nsimul,1))
     mask = data(:,tdiscard:tsimul1,9).gt.1.5d0
  ...
  open(3,file='plot_data/trash_this.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_PU.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_PUT.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_PUTebkacc.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_PUTebkprov.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_PUTabkacc.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_PUTabkprov.txt',position="rewind")
  ...
  print*,""
  allocate(mask(nsimul,107-103+1))
  mask = data(:,103:107,9).gt.1.5d0
  meanIK = sum(data(:,103:107,12),mask)/dble(count(mask))
  meanLP = sum(data(:,103:107, 7),mask)/dble(count(mask))
  meanLP2= sum(data(:,103:107,16),mask)/sum(data(:,103:107,6),mask)
  meanAV = sum(data(:,103:107,20),mask)/dble(5)
  meanAP = sum(data(:,103:107,18),mask)/dble(5)
  meanAI = sum(data(:,103:107,10),mask)/dble(5)
  meanAD = sum(data(:,103:107,16),mask)/dble(5)

  allocate(mask2(nsimul))
  mask2 = data(:,110,9).gt.1.5d0
