program greek_crisis_exp
  ...
  type(param) par
  integer, parameter:: tsimul = 125, nsimul = 100000, nvars = 25
  real(dp), parameter::stimul=0.02d0
  integer j,i,n,t,tdiscard,tsimul1,iloc(1),tt0,tt1,iflag_turnashocks,iflag_turnxshocks,iflag_creditshock,iflag_saved
  integer iflag_debt,iflag_debt_all,iflag_sales,iflag_rev,il0,il1,ip
  real(dp) ee,sparam,tau,taudiv
  integer,  allocatable:: iapath(:),ixpath(:)
  real(dp), allocatable:: shocksz(:,:),shocksa(:),shocksx(:),data(:,:,:),initconds(:,:)
  logical, allocatable:: mask(:,:),mask2(:),mask3(:,:)
  ...
  real scale_param,lev50,lev25,lev75

  real(dp) profits0,gsales,gdebt,rev12,rev12plus,revaux1,revaux0,profits12,profitsnp12,sumcost,mdrop,belig
  ...
  ! HOW IT WORKS: TRIGER () (sales) (debt) (debt.debt_all) (rev) (rev.debt) (rev.debt.debt_all)
  iflag_rev         = 1
  iflag_sales       = 0
  iflag_debt        = 1
  iflag_debt_all    = 1
  ...
  if(iflag_turnashocks.eq.1)then
     ...
     iloc = minloc(abs( 0.08d0 - m%a)); iapath(108) = iloc(1)
     iloc = minloc(abs( 0.03d0 - m%a)); iapath(109) = iloc(1)
     iloc = minloc(abs(-0.11d0 - m%a)); iapath(110) = iloc(1)
     iloc = minloc(abs(-0.16d0 - m%a)); iapath(111) = iloc(1)

     if(iflag_sales.eq.0)then
        iloc = minloc(abs(-0.18d0 - m%a)); iapath(112) = iloc(1)
     else
        iloc = minloc(abs(-0.18d0+stimul - m%a)); iapath(112) = iloc(1)
     end if

     iloc = minloc(abs(-0.05d0 - m%a)); iapath(113) = iloc(1)
     iloc = minloc(abs( 0.00d0 - m%a)); iapath(114) = iloc(1)
  end if
  ...
  if(iflag_creditshock.eq.0)then
     ...
     !ee = 0.0d0	! TO SIMULATE MORE PRONOUNCED CREDIT SUPPLY SHOCKS - BASELINE
     !ee = 0.40d0	! TO SIMULATE MORE PRONOUNCED CREDIT SUPPLY SHOCKS - VSEVERE 0.41
     ee = -0.40d0	! TO SIMULATE MORE PRONOUNCED CREDIT SUPPLY SHOCKS - LSEVERE -0.5
     
     ! ! SPARAM UNDER BASELINE
     ! m_2%sparam = sparam*(1d0 + 0.002d0*(1d0+ee))! +0.05d0
     ! m_1%sparam = sparam*(1d0 - 0.002d0*(1d0+ee))! +0.05d0
     ! m0%sparam  = sparam*(1d0 - 0.012d0*(1d0+ee))! +0.05d0
     ! m1%sparam  = sparam*(1d0 - 0.030d0*(1d0+ee))! +0.05d0
     ! m2%sparam  = sparam*(1d0 - 0.058d0*(1d0+ee))! +0.05d0
     ! m3%sparam  = sparam*(1d0 - 0.080d0*(1d0+ee))! +0.05d0
     ! m4%sparam  = sparam*(1d0 - 0.087d0*(1d0+ee))! +0.05d0

     ! SPARAM UNDER PROVISION
     m_2%sparam = sparam*(1d0 + 0.002d0*(1d0+ee))!+ 0.05d0
     m_1%sparam = sparam*(1d0 - 0.008d0*(1d0+ee))!+ 0.05d0
     m0%sparam  = sparam*(1d0 - 0.030d0*(1d0+ee))!+ 0.05d0
     m1%sparam  = sparam*(1d0 - 0.060d0*(1d0+ee))!+ 0.05d0
     m2%sparam  = sparam*(1d0 - 0.120d0*(1d0+ee))!+ 0.05d0
     m3%sparam  = sparam*(1d0 - 0.170d0*(1d0+ee))!+ 0.05d0
     m4%sparam  = sparam*(1d0 - 0.200d0*(1d0+ee))!+ 0.05d0
     ...
     m0%tau = 0.24d0
     m1%tau = 0.20d0
     if(iflag_rev.eq.1.and.iflag_debt.eq.0)then
        m2%tau = 0.01d0
     else
        m2%tau = 0.2d0
     end if
     ...
     do tt0 = 108,114
        ...
        if(iflag_debt.eq.1)then
           if(tt0.eq.112)then              
              call sampp(real(data(:,111,7)),nsimul,0.40,0,lev25)

              if(iflag_debt_all.eq.0)then
                 call sampp(real(data(:,111,7)),nsimul,0.5,0,lev50)
              else
                 lev50 = 0.00
              end if
              call sampp(real(data(:,111,7)),nsimul,0.75,0,lev75)

              if(iflag_rev.eq.1)then
                 open(1,file='data/rev12.txt',position="rewind");read(1,*)revaux0;close(1)
                 open(1,file='data/rev12_pol.txt',position="rewind");read(1,*)revaux1;close(1)
                 gsales = revaux0-revaux1
              else
                 open(1,file='data/gsales.txt',position="rewind");read(1,*)gsales;close(1)
              end if

              !print*,'active',count(data(:,111,9).gt.0.5d0)
              !print*,'leverages',lev25,lev50,lev75
              !print*,'debts',sum(data(:,111,7)*data(:,111,6))/1d6,sum(data(:,111,7)*data(:,111,6),data(:,111,7).ge.real(lev50,8)-0.0001d0)/1d6              
              belig = sum(data(:,111,7)*data(:,111,6),data(:,111,7).ge.real(lev50,8)-0.000001d0)/1d6
              ...
              i = 1
              j = 0
              do while(i.le.nsimul.and.sumcost.le.gsales*1d6)
                 if(data(i,111,7).ge.real(lev50,8)-0.0001d0) then
                    
                    iloc = minloc(abs((1d0-mdrop)*data(i,111,7) - m%p)); ip = iloc(1)

                    if(m%p(ip).gt.(1d0-mdrop)*data(i,111,7)) ip = ip - 1 	!MAY NEED TO TURN THIS ON

                    sumcost = sumcost + (data(i,111,7)-m%p(ip)) * data(i,111,6)
                    initconds(i,2) = m%p(ip)
                    j = j + 1
                 end if
                 i=i+1
              end do
              ! i = 1
              ! do while(i.le.nsimul.and.sumcost.le.gsales*1d6)
              !    if(data(i,111,7).ge.real(lev50,8)-0.001d0.and.data(i,111,7).lt.real(lev50,8)-0.0001d0) then
              !       iloc = minloc(abs((1d0-mdrop)*data(i,111,7) - m%p)); ip = iloc(1)
              ...
              !print*,'cost',sumcost/1d6

              !stop 'hey!!'
              print*,'hey!!!',nsimul,i,j
           end if
        end if
  ...
  do t=101,tsimul
     ...
     write(*,'(1i4,1i8,22f11.4)')t,count(mask),m%a(iapath(t)),m%xsigmaz(ixpath(t)), &
          ...
          statistics(24),statistics(29)/scale_param, &
  ...
  !open(3,file='plot_data/Distributions_cal_estimation_base_nopol.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_base_sales.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_base_rev.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_base_debt.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_base_debt2.txt',position="rewind")

  !open(3,file='plot_data/Distributions_cal_estimation_vsev_nopol.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_vsev_sales.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_vsev_rev.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_vsev_debt.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_vsev_debt2.txt',position="rewind")

  !open(3,file='plot_data/Distributions_cal_estimation_lsev_nopol.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_lsev_sales.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_lsev_rev.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_lsev_debt.txt',position="rewind")
  !open(3,file='plot_data/Distributions_cal_estimation_lsev_debt2.txt',position="rewind")

  do t=101,tsimul ! SEE LEGACY CODES FOR CONSISTENCY
     do n=1,nsimul
        write(3,'(2i10,22f16.5)')t,n,m%sparam, &
             1.0d0-0d0,data(n,t,19),data(n,t,12), &
             data(n,t,23),data(n,t,21),data(n,t,5),data(n,t,3),data(n,t,4),data(n,t,18), &
             data(n,t,6),data(n,t,7),data(n,t,25)
     end do
  end do
  write(3,'(//)')
  close(3)
  ...
  profits0 = sum(data(:,103:107,18))/dble(5)
  ...
  allocate(mask3(nsimul,tsimul-112+1))
  mask3 = data(:,112:tsimul,9).gt.0.5d0

  rev12     = sum(data(:,112,25),mask2)
  rev12plus = sum(data(:,112:tsimul,25),mask3)  
  deallocate(mask3)
  
  if(iflag_sales.eq.1)then
     iloc = minloc(abs(-0.18d0 - m%a)); il0 = iloc(1)
     iloc = minloc(abs(-0.18d0+stimul - m%a)); il1 = iloc(1)
     profits12   = sum(data(:,112,18),mask2)
     profitsnp12 = profits12*exp(m%a(il0)-m%a(il1))
  end if
  ...
  if(iflag_sales.eq.1)then
     gsales = 2d0*(profits12-profitsnp12)/1d6
     write(*,'(3a16)')'Gsales','InitSales','%Gsales'
     write(*,'(3f16.4)')gsales,2d0*profits0/1d6,(profits12-profitsnp12)/profits0
     open(1,file='data/gsales.txt',position="rewind");write(1,*)gsales;close(1)
  end if

  if(iflag_debt.eq.1)then
     gdebt = sumcost/1d6
     write(*,'(3a16)')'Gdebt','m','%Gdebt'
     write(*,'(3f16.4)')gdebt,mdrop,gdebt/(2d0*profits0/1d6)
     open(1,file='data/gdebt.txt',position="rewind");write(1,*)gdebt;close(1)
  end if

  if(iflag_debt.eq.0.and.iflag_sales.eq.0)then
     print*,''
     print*,''
  end if
  if(iflag_rev.eq.0)then
     write(*,'(5a16)')'TRev12','TRev12plus','ISales','%Rsales12','%Rsales12plus'
     write(*,'(5f16.4)')rev12/1d6,rev12plus/1d6,2d0*profits0/1d6,0.5d0*rev12/profits0,0.5d0*rev12plus/profits0
  elseif(iflag_rev.eq.1)then
     open(1,file='data/rev12.txt',position="rewind");read(1,*)revaux0;close(1)
     write(*,'(7a16)')'TRev12','TRev12plus','ISales','%Rsales12','%Rsales12plus','DR','%DRsales'
     write(*,'(7f16.4)')rev12/1d6,rev12plus/1d6,2d0*profits0/1d6,0.5d0*rev12/profits0,0.5d0*rev12plus/profits0,revaux0-rev12/1d6,0.5d0*(revaux0*1d6-rev12)/profits0
  end if
  if(iflag_rev.eq.0)then
     open(1,file='data/rev12.txt',position="rewind");write(1,*)rev12/1d6;close(1)
     open(1,file='data/rev12plus.txt',position="rewind");write(1,*)rev12plus/1d6;close(1)
  elseif(iflag_rev.eq.1.and.iflag_debt.eq.0)then
     open(1,file='data/rev12_pol.txt',position="rewind");write(1,*)rev12/1d6;close(1)
     open(1,file='data/rev12plus_pol.txt',position="rewind");write(1,*)rev12plus/1d6;close(1)
  end if
