program stats
  ...
  type(model) m
  type(param) par
  integer, parameter:: tsimul = 107, tdiscard = 103, nsimul = 50000, nvars = 25
  ...
  if(iflag_asindata.eq.1)then
     ...
     call get_simulations(par,m,nsimul,tsimul,nvars,shocksz,shocksa,shocksx,data,iapath=iapath,ixpath=ixpath,replace_exit='y')
  else
     call get_simulations(par,m,nsimul,tsimul,nvars,shocksz,shocksa,shocksx,data,replace_exit='y')
  end if

  if(iflag_saveshocks.eq.1)then
     open(1,file='functions/shocksz.dat',position="rewind");write(1,*)shocksz;close(1)
     open(1,file='functions/shocksa.dat',position="rewind");write(1,*)shocksa;close(1)
     open(1,file='functions/shocksx.dat',position="rewind");write(1,*)shocksx;close(1)
     open(1,file='functions/iapath.dat',position="rewind");write(1,*)iapath;close(1)
     open(1,file='functions/ixpath.dat',position="rewind");write(1,*)ixpath;close(1)
  end if
  ...
  open(1,file='data/e_p.txt',position="rewind");write(1,*)statistics(4);close(1)
