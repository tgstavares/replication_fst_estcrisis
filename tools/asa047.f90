subroutine nelmin ( fn, n, xlb, xub, start, xmin, ynewlo, reqmin, step, konvge, kcount, &
  ...
  !    0 < N is required.
  !   
  !   (added) xlb ( kind = 8 ) lower bound (input)
  !   (added) xub ( kind = 8 ) upper bound (input)
  !
  ...
  do j=1,n
     if(start(j).lt.xlb(j)) then
        print*,start
        stop 'erro - bound start lb'
     end if
     if(start(j).gt.xub(j)) then
        print*,start
  ...
  do
     ...
     do j = 1, n
        ...
        if(start(j).lt.xlb(j)) start(j) = xlb(j)
        if(start(j).gt.xub(j)) start(j) = xub(j)
     ...
     do while ( icount < kcount )
        ...
        do i=1,n
           if(pstar(i).lt.xlb(i)) pstar(i) = xlb(i)
           if(pstar(i).gt.xub(i)) pstar(i) = xub(i)
        end do
        ...
        if ( ystar < ylo ) then
           ...
           do i=1,n
              if(p2star(i).lt.xlb(i)) p2star(i) = xlb(i)
              if(p2star(i).gt.xub(i)) p2star(i) = xub(i)
           end do
